function PasswordGenerator(props) {
  return (
    <div className=" w-full bg-teal-900 pt-10 h-screen flex flex-col items-center">
      <section className="bg-yellow-200 p-4 w-[450px] shadow-md">
        ...
        <span>
          ...
          <button
            type="button"
            className="bg-blue-700 text-white w-[80px] p-1 text-center"
            onClick={() => {
              props.copy(props.pass);
            }}
          >
            {props.showCopy ? "Copied" : "Copy"}
          </button>
        </span>
        <section className="mt-2 flex justify-between">
          <label htmlFor="length" className="flex">
            <input
          ...
          <label htmlFor="numId" className="flex">
            <input
              id="numId"
              checked={props.NumAllowed}
              onChange={() => {
                props.setnumAllowed((previous) => !previous);
              }}
              type="checkbox"
            />
            Numbers
          ...
          <label htmlFor="charId" className="flex">
            <input
              id="charId"
              checked={props.charAllowed}
              onChange={() => {
                props.setCharAllowed((previous) => !previous);
              }}
              type="checkbox"
            />
            Characters
